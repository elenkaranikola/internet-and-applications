...
for index, js in enumerate(json_files):
    with open(os.path.join(path_to_json, js)) as json_file:
        ...
        for spot, j in enumerate(json_text['body_text']):
            if json_text['body_text'][spot]['ref_spans'] != []:
                for count, k in enumerate(json_text['body_text'][spot]['ref_spans']):
                    start = json_text['body_text'][spot]['ref_spans'][count]['start']
                    end = json_text['body_text'][spot]['ref_spans'][count]['end']
                    text = json_text['body_text'][spot]['ref_spans'][count]['text']
                    ref_id = json_text['body_text'][spot]['ref_spans'][count]['ref_id']
                    ref_data = (paper_id,spot,start,end,text,ref_id)
                    add_data = ("INSERT IGNORE INTO ref_span "
